...
while :; do
	...
	if [ $((cnt_tmb++)) -ge ${upd_tmb} ]; then
		...
		if [ "${BAT0}" != "" ] || [ "${BAT1}" != "" ]; then
			...
			a=($(paste -d = "/sys/class/power_supply/BAT0/uevent" 2>/dev/null | awk '/STATUS=/||/CHARGE_NOW=/||/CHARGE_FULL=/' | cut -d '=' -f 2))
			if [ ${a[0]} = "Discharging" ]
			then
				CHARGE="D"
			elif [ ${a[0]} = "Charging" ]
			...
			TIME=($(acpi -b | awk '/[0-9][0-9]:[0-9][0-9]:[0-9][0-9]/ {if($5!=""){TIME=$5}i} END {if(TIME!=""){print TIME} else {print "none"}}'))
			printf "%s%s\n" "TMB" "$PERC $CHARGE $TIME" > "${panel_fifo}"

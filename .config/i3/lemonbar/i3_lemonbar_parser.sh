...
while read -r line ; do
	case $line in
		...
		NET*)
			...
			local_ip="%{F${color_sec_b2}}${sep_left}%{F${color_icon} B${color_sec_b2}} %{T2}${net_icon}%{F- T1} ${net_arr_ip}"

			## Wifi Signal Strength
			if [ ${net_arr_signal} != "none" ]; then
				wifi="%{F${color_icon}}${sep_l_left}%{F${color_icon} B${color_sec_b2}} %{T2}${icon_wifi}%{F- T1} ${net_arr_signal}%"
			  else
				wifi=""
			fi
			...
			if [ "${net_arr_ip}" = "No IP" ]; then
				ext_ip="%{F${color_sec_b1}}${sep_left}%{F${color_icon} B${color_sec_b1}} %{T2}${icon_ext_ip}%{F- T1} No IP"
			fi
			;;
		### End Network Case ### }}}
		...
		EXT*)
			...
			ext_ip="%{F${color_sec_b1}}${sep_left}%{F${color_icon} B${color_sec_b1}} %{T2}${icon_ext_ip}%{F- T1} ${ext_ip_select}"
			;;
